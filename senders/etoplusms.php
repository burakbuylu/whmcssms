<?php
class etoplusms extends AktuelSms {
	...
	function send(){
		...
		'<SMS>'.
			...
			'</mesaj>'.
		'</SMS>';
		$URL = "http://www.dakikapi.com//api/xml_api.php";
		$options = array(
			"CURLOPT_MUTE" => 1,
        ...
        $result = curlCall($URL, $xml_data, $options);
	...
	function balance(){
		...
		if($params->user && $params->pass){
			...
			$URL = "http://www.dakikapi.com/api/xml_bakiye.php"; 
			$ch = curl_init($URL);
	...
	function report($msgid){
		...
		if($params->user && $params->pass && $msgid){
			...
			'<RAPOR>'.
				...
				'</rapor>'.
			'</RAPOR>';
			$URL = "http://www.dakikapi.com/api/xml_rapor.php"; 
			$ch = curl_init($URL);
			curl_setopt($ch, CURLOPT_MUTE, 1);
			curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
